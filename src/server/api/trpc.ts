...
//   });
  ...
  .use(async ({ ctx, next }) => {
    ...
    if (!authHeader.startsWith("Bearer ")) {
      throw new TRPCError({ code: "UNAUTHORIZED", message: "Bearer not Bearer" });
    }

    const authHeaderSplited = authHeader.split(" ");

    if (authHeaderSplited.length < 2) {
      throw new TRPCError({ code: "UNAUTHORIZED", message: "Invalid token? U need new token, bastard" });
    }
  ...
  .use(timingMiddleware)
  .use(async ({ ctx, next }) => {
    const match = await ctx.db.match.findMany();
    return next({
      ctx: {
        ...ctx,
        match,
  ...
  export const userGetAllProcedure = t.procedure
  .use(timingMiddleware)
  .use(async ({ ctx, next }) => {
