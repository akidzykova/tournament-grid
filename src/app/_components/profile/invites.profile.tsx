...
export const InvitesProfile: React.FC = () => {
    const [tab, setTab] = useState<Tab>(Tab.MATCH);
    ...
    return <>
        <Section>
            <NavButton
                active={tab === Tab.MATCH}
                onClick={() => setTab(Tab.MATCH)}
            >
                Матчи
            </NavButton>
            <NavButton
                active={tab === Tab.ORGANIZATION}
                onClick={() => setTab(Tab.ORGANIZATION)}
            >
                Организации
            </NavButton>
        </Section>
...
const MatchInvites: React.FC = () => {
    const {addNotification} = useNotifications();

    const inviteGetQuery = api.invite.get.useQuery({
        skip: 0,
        take: 50,
    });
    ...
    const inviteAcceptMutation = api.invite.accept.useMutation();
    useEffect(() => {
    ...
    const inviteRejectMutation = api.invite.reject.useMutation();
    useEffect(() => {
    ...
    return (
        <>
            ...
            <Wrapper>
                <UIH1>
                    Приглашения в матчи <IconButton size={24} icon={IconEnum.REFRESH} onClick={refresh} />
                </UIH1>
                {
                        ...
                        <NotificationsList>
                            {inviteGetQuery.data && inviteGetQuery.data.length > 0
                                ? inviteGetQuery.data.map((invite) => (
                                    ...
                                    >
                                        <Title>
                                            Матч &#34;{invite.match.name}&#34; от организации &#34;
                                            {invite.match.organization.name}&#34;
                                        </Title>
                                        <Text>Начнется {formatEventDate(invite.match.startAt)}</Text>
                                        <Date>{invite.createAt.toLocaleString()}</Date>
                                    </InviteCard>
                                ))
...
const OrganizationInvites: React.FC = () => {
    const {addNotification} = useNotifications();

    const inviteGetQuery = api.organizationInvite.get.useQuery({
        skip: 0,
        take: 50,
    });
    ...
    const inviteAcceptMutation = api.organizationInvite.accept.useMutation();
    useEffect(() => {
        if (inviteAcceptMutation.isError) {
            addNotification("Произошла ошибка", NotificationTypeEnum.ERROR);
        }
    ...
    const inviteRejectMutation = api.organizationInvite.reject.useMutation();
    useEffect(() => {
        if (inviteRejectMutation.isError) {
            addNotification("Произошла ошибка", NotificationTypeEnum.ERROR);
        }
    ...
    return (
        <>
            ...
            <Wrapper>
                <UIH1>
                    Приглашения в организации <IconButton size={24} icon={IconEnum.REFRESH} onClick={refresh} />
                </UIH1>
                {
                    inviteGetQuery.isLoading || inviteGetQuery.isRefetching ?
                        <Loader type='fluid' /> :
                        <NotificationsList>
                            {inviteGetQuery.data && inviteGetQuery.data.length > 0
                                ? inviteGetQuery.data.map((invite) => (
                                    ...
                                    >
                                        <Title>
                                            Организация &#34;{invite.organization.name}&#34;
                                        </Title>
                                        <Date>{invite.createAt.toLocaleString()}</Date>
                                    </InviteCard>
                                ))
